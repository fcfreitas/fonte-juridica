...
export async function POST(request: Request) {
  ...
  const mailOptions = {
    from:`"Jusdex" <${process.env.EMAIL_USER}>`,
    to: email,
    subject: "Recuperação de Senha - Jusdex",
      ...
      <div style="font-family:Arial, sans-serif; background-color:#f8fafc; color:#0f172a; padding:40px 20px;">
        <div style="max-width: 600px; margin: 0 auto; background: #ffffff; padding: 30px; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
          ...
          <p style="font-size: 16px; line-height: 1.6; color: #334155;">
            Recebemos uma solicitação para redefinir a senha da sua conta no <strong>Jusdex</strong>.
          </p>
          <p style="font-size: 16px; line-height: 1.6; color: #334155;">
            Se foi você, clique no botão abaixo. O link é válido por 1 hora.
          </p>
          <div style="text-align: center; margin: 30px 0;">
            <a href="${resetLink}" style="background-color:#14161C; color:#CCA25E; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold;">
              Redefinir senha
            </a>
          </div>
          ...
          <p style="font-size: 12px; color: #94a3b8; text-align: center;">
            © ${new Date().getFullYear()} Jusdex. Todos os direitos reservados.
          </p>
        </div>
      </div>
    `,
