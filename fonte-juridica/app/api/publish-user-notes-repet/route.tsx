...
export async function POST(request: Request) {
  try {
    ...
    await db.collection("repetitivosUserPrivateNotes").insertOne({
...
export async function GET(req: Request) {
  try {
    ...
    const posts = await db.collection("repetitivosUserPrivateNotes").find(query).toArray();
...
export async function PUT(req: Request) {
  try {
    ...
    const existing = await db.collection("repetitivosUserPrivateNotes").findOne({ _id: new ObjectId(id) });
    if (!existing || existing.userId !== session.user.id) {
      return NextResponse.json({ message: "Acesso negado!" }, { status: 403 });
    }

    await db.collection("repetitivosUserPrivateNotes").updateOne(
      { _id: new ObjectId(id) },
