...
interface JulgadosPageClientProps {
  initialJulgados?: Julgado[];
  initialTotal?: number;
}

export default function JulgadosPageClient({ initialJulgados, initialTotal }: JulgadosPageClientProps) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [isOpen, setIsOpen] = useState(false);

  const { ramoDireito, setRamoDireito, assunto, setAssunto, situacaoRepGeral, setSituacaoRepGeral, situacaoTema, setSituacaoTema, searchText, searchTema } = useFilter();
  ...
  return (
    ...
    {/*Versão Desktop */}
      ...
      <div className={assinaturaValida ? "" : "blur-sm pointer-events-none select-none"}>
        <JulgadosList initialJulgados={initialJulgados} initialTotal={initialTotal} />
      </div>
