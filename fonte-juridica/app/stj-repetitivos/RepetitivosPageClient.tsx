...
import { Repetitivo } from "@/app/julgados-data";

interface RepetitivosPageClientProps {
  initialRepetitivos?: Repetitivo[];
  initialTotal?: number;
}

export default function RepetitivosPageClient({ initialRepetitivos, initialTotal }: RepetitivosPageClientProps) {
  const { data: session, status } = useSession();
  const router = useRouter();
  const [isOpen, setIsOpen] = useState(false);

  const { ramoDireito, setRamoDireito, assunto, setAssunto, situacaoTema, setSituacaoTema, searchText, searchTema } = useFilter();
  ...
  return (
    ...
    {/*Versão Desktop */}
      <div className="bg-slate-50 w-full flex items-center">
        <h2 className="text-xl font-semibold leading-none text-2xl md:text-3xl font-bold text-slate-800">STJ - Repetitivos</h2>
      </div>
      ...
      <div className="group flex w-full pt-8">
        ...
        {/* Lista de repetitivos */}
        <main className="flex-1 px-4 md:px-6">
        <div className="relative">
      <div className={assinaturaValida ? "" : "blur-sm pointer-events-none select-none"}>
        <RepetitivosList initialRepetitivos={initialRepetitivos} initialTotal={initialTotal} />
      </div>
